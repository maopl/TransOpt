...
class LunarlanderBenchmark(ContinuousOptBenchmark):
    '''
    DixonPrice function
    ...
    def __init__(self,task_name, task_id, budget, seed, task_type='non-tabular'):
        ...
        super(LunarlanderBenchmark, self).__init__(task_name=task_name, seed=seed,
    ...
    def get_configuration_space(self, seed: Union[int, None] = None) -> CS.ConfigurationSpace:
        ...
        cs.add_hyperparameters([CS.UniformFloatHyperparameter(f'x{i}', lower=0, upper=2.) for i in range(10)])
    ...
    for i, vector1 in enumerate(result_vectors):
        for j, vector2 in enumerate(result_vectors):
            if i != j:
                ...
                print(f"Correlation between seed {seed_list[i]} and seed {seed_list[j]}: {correlation},p_{p}")
