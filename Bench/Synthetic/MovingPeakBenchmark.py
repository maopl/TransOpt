...
class MovingPeakGenerator:
    def __init__(self, n_var, shift_length=3.0, height_severity=7.0, width_severity=1.0, lam=0.5, n_peak=4, n_step=11, seed=None):
        if seed is not None:
            np.random.seed(seed)
        self.n_var = n_var
        ...
        self.bounds = np.array([[-1.0] * self.n_var, [1.0] * self.n_var],  dtype=np.float64)


        current_peak = np.random.random(size=(n_peak, n_var)) \
        ...
        previous_shift = normalize(np.random.random(size=(n_peak,n_var)),axis=1, norm='l2')
    ...
    def cal_peak_shift(self,previous_shift):
        ...
        return (1 - self.lam) * self.shift_length * normalize(
            peak_change - 0.5, axis=1, norm='l2') + self.lam * previous_shift
    ...
    def transfer(self, X):
        return (X + 1) * (self.var_bound[:, 1] - self.var_bound[:, 0]) / 2 + (self.var_bound[:, 0])

    def normalize(self, X):
        return 2 * (X - (self.var_bound[:, 0]))/(self.var_bound[:, 1] - self.var_bound[:, 0]) - 1
...
class MovingPeakBenchmark(ContinuousOptBenchmark):
    def __init__(self, task_name, budget, peak, height, width, seed, input_dim, task_type='non-tabular'):
        self.dimension = input_dim
        self.peak = peak
        self.height = height
        self.width = width
        self.n_peak = len(peak)
        super(MovingPeakBenchmark, self).__init__(task_name=task_name, seed=seed, task_type=task_type, budget=budget)
    ...
    def peak_function_hilly(self,x):
        distance = np.linalg.norm(np.tile(x, (self.n_peak, 1)) - self.peak, axis=1)
        return  np.max(self.height - self.width * distance * distance - 0.01 * np.sin(20.0 * distance * distance))
    ...
    def get_configuration_space(self, seed: Union[int, None] = None) -> CS.ConfigurationSpace:
        ...
        cs.add_hyperparameters([CS.UniformFloatHyperparameter(f'x{i}', lower=0., upper=100.) for i in range(self.dimension)])
