...
class BenchmarkBase(abc.ABC, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def f(self, configuration: Union[ConfigSpace.Configuration, Dict],
    ...
    def check_parameters(wrapped_function):
        ...
        def wrapper(self, configuration: Union[ConfigSpace.Configuration, Dict],
            ...
            configuration = BenchmarkBase._check_and_cast_configuration(configuration, self.configuration_space)

            # Second, evaluate the given fidelities.
            # Sanity check that there are no fidelities in **kwargs
            fidelity = BenchmarkBase._check_and_cast_fidelity(fidelity, self.fidelity_space, **kwargs)
