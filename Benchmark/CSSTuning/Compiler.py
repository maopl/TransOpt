...
class GCC(NonTabularOptBenchmark):
    def __init__(self, task_name, budget, seed, task_id, task_type='non-tabular', **kwargs):
        ...
        super(GCC, self).__init__(task_name=task_name, seed=seed, task_type=task_type, budget=budget,
                                  task_id=task_id)
        np.random.seed(seed)
    ...
    def objective_function(self, configuration: Union[ConfigSpace.Configuration, Dict],
        ...
        try:
            ...
            end_time = time.time()
            return {'function_value': float(performance['avrg_exec_time']),
            ...
            return {'function_value': float(ERROR_VALUE),
    ...
    def get_configuration_space(self, seed: Union[int, None] = None) -> ConfigSpace.ConfigurationSpace:
        ...
        for key, value in config.items():
            if value['type'] == 'enum':
                ...
                hyperparameters.append(
                    CS.UniformIntegerHyperparameter(key, lower=value['range'][0], upper=value['range'][1],
...
class LLVM(NonTabularOptBenchmark):
    def __init__(self, task_name, budget, seed, task_id, task_type='non-tabular', **kwargs):
        ...
        self.config_knob = self.benchmark.config_space.get_all_details()
        super(LLVM, self).__init__(task_name=task_name, seed=seed, task_type=task_type, budget=budget,
                                  task_id=task_id)
        np.random.seed(seed)
