...
class ELLA(object):
    ...
    def fit(self, X, y, task_id):
        ...
        # if self.T != 1:
        #     np.random.seed(1)
        #     random_num = np.ones(shape=(self.T - 1,))
        #     sum = self.T - 1
        #     loss_weight = np.append(random_num, sum)
        #     loss_weight = loss_weight / (2 * sum)
        # else:
        #     loss_weight = 1
        loss_weight = 2
        ...
        # L_vectorized = inv(self.A / loss_weight + self.lam * np.eye(self.d * self.k, self.d * self.k)).dot(self.b) / loss_weight
