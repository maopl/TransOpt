...
from Optimizer.OptimizerAbstract import OptimizerAbs
import GPyOpt
...
class BOAbs(OptimizerAbs):
    """
    The abstract Model for Bayesian Optimization
    """

    def __init__(self, config):
        super(BOAbs, self).__init__(config=config)
        ...
        if 'ini_ratio' in config:
            self.ini_ratio = config['ini_ratio']
        else:
            self.ini_ratio = 4
    ...
    def set_space(self, design_space_info: Dict[str, dict], search_space_info: Union[Dict[str, dict], None]= None):
        ...
        if self.ini_num is None:
            self.ini_num = self.ini_ratio * self.input_dim
