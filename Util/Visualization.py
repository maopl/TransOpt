...
def visual_contour(optimizer, testsuites, ac_model,
    ...
    xgrid_0, xgrid_1 = np.meshgrid(np.linspace(search_bound[0][0], search_bound[0][1], test_size, dtype=dtype),
    ...
    ax_plot('iter_' + str(train_x.shape[0]), ax[0][0], train_x, test_y.reshape(xgrid_0.shape), test_size, cm.Reds)

    ax_plot('Prediction', ax[0][1], train_x, observed_pred_y.reshape(xgrid_0.shape), test_size, cm.Blues)
    ...
    if not os.path.exists('{}/verbose/contour/{}/{}'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}')):
        os.makedirs('{}/verbose/contour/{}/{}'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}'))

    plt.savefig('{}/verbose/contour/{}/{}/{}.png'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}', f'iter_{testsuites.get_query_num()}'), format='png')
    plt.close()
    testsuites.unlock()
...
def visual_oned(optimizer, testsuites, ac_model,
    ...
    Exper_folder = optimizer.exp_path
    if not os.path.exists('{}/verbose/oneD/{}/{}'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}')):
        os.makedirs('{}/verbose/oneD/{}/{}'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}'))

    ax.legend()
    plt.grid()

    plt.savefig('{}/verbose/oneD/{}/{}/{}.png'.format(Exper_folder, optimizer.optimizer_name, f'{testsuites.get_curname()}', f'iter_{testsuites.get_query_num()}', format='png'))
