...
def run_experiments(tasks, args):
    ...
    while(testsuits.get_unsolved_num()):
        ...
        while(testsuits.get_rest_budget()):
            ...
            optimizer.sync_from_handler(data_handler)
            optimizer.set_auxillary_data(data_handler.get_auxillary_data())
        testsuits.roll()


if __name__ == '__main__':
    tasks = {
             'Ackley': {'budget': 110, 'time_stamp': 3, 'params':{'input_dim':10}},
             # 'MPB': {'budget': 110, 'time_stamp': 3},
             # 'Griewank': {'budget': 110, 'time_stamp': 3,  'params':{'input_dim':10}},
    ...
    parser.add_argument("-in", "--init_number", type=int, default=40)
    ...
    parser.add_argument("-m", "--optimizer", type=str, default='LFL')  # 设置method:WS,MT,INC
