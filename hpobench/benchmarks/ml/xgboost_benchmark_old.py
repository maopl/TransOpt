...
class XGBoostBenchmark(AbstractBenchmark):
    ...
    def get_configuration_space(seed: Union[int, None] = None) -> CS.ConfigurationSpace:
        ...
        cs.add_hyperparameters([
            ...
            CS.UniformIntegerHyperparameter('max_delta_step', lower=0, upper=10, default_value=0, log=False),
            CS.UniformFloatHyperparameter('gamma', lower=0, upper=1, default_value=0, log=False)
        ])
    ...
    def _get_pipeline(self, max_depth: int, eta: float, min_child_weight: int,
                      ...
                      n_estimators: int, subsample_per_it: float, max_delta_step: int, gamma: float) \
            -> pipeline.Pipeline:
