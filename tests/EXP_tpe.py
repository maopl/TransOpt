...
def obj_fun(eval_config: dict[str, float]) -> tuple[dict[str, float], float]:
    start = time.time()
...
class formal_obj:
    ...
    def new_f(self, configuration):
        ...
        return {'loss': results['function_value']}, time.time() - start
...
for nn_name in NN_name:
    for w in workloads:
        ...
        opt = TPEOptimizer(obj_func=obj_f.new_f, config_space=cs, n_init=22, max_evals=100, resultfile='nn_name')
