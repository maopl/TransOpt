...
@analysis_registry.register('traj')
def plot_traj(ab:AnalysisBase, save_path:Path):
    traj_path = save_path / 'traj'
    for experiment_name, datasets in ab._all_data.items():
        plt.clf()
        plt.figure(figsize=(12, 6))
        for dataset in datasets:
            problem_name = ab._data_infos[experiment_name][dataset]['additional_config']['problem_name']
            workloads = ab._data_infos[experiment_name][dataset]['additional_config']['workloads']
            model = ab._data_infos[experiment_name][dataset]['additional_config']['Model']
            variable_name = [i['name'] for i in ab._data_infos[experiment_name][dataset]['variables']]
            objective_name = [i['name'] for i in ab._data_infos[experiment_name][dataset]['objectives']]
            name = f"{problem_name}_{workloads}_{model}"
            data = ab._all_data[experiment_name][dataset]
            Y = np.array([i[objective_name[0]] for i in data])
            Y = np.minimum.accumulate(Y)
            iters = np.array(list(range(len(Y))))
            plt.plot(iters, Y, label=name)
        plt.legend()
        os.makedirs(traj_path, exist_ok=True)
        plt.savefig(traj_path / f'traj_{experiment_name}.png')


@analysis_registry.register('best')
def plot_best(ab:AnalysisBase, save_path:Path):
    traj_path = save_path / 'best'
    for experiment_name, datasets in ab._all_data.items():
        plt.clf()
        plt.figure(figsize=(12, 6))
        ...
        for dataset in datasets:
            ...
            data = ab._all_data[experiment_name][dataset]
            Y = np.array([i[objective_name[0]] for i in data])
            best_output.append(np.min(Y))
        iters = np.array(list(range(len(best_output))))
        plt.plot(iters, best_output)
        plt.legend()
        os.makedirs(traj_path, exist_ok=True)
        plt.savefig(traj_path / f'best_{experiment_name}.png')
    
@analysis_registry.register('Plasticity_KDE')
def plot_plasticity_KDE(ab:AnalysisBase, save_path:Path):    
    traj_path = save_path / 'plasticity_kde'
    plt.figure(figsize=(12, 6))
    ...
    for experiment_name, datasets in ab._all_data.items():
        for dataset in datasets:
            ...
            # Get X and Y data
            X = np.array([[d[var] for var in variable_name] for d in data])
            Y = np.array([i[objective_name[0]] for i in data])
            
            # Find best output and corresponding X
            best_output = np.min(Y)
            ...
            # Get KDE value at best X location
            kde_value = np.exp(kde.score_samples([best_x])[0])
            
            kde_values.append(kde_value)
            best_outputs.append(best_output)
            
        # Sort best_outputs and kde_values together
        sorted_indices = np.argsort(best_outputs)
        best_outputs = np.array(best_outputs)[sorted_indices]
        kde_values = np.array(kde_values)[sorted_indices]
    
        plt.scatter(best_outputs, kde_values)
        plt.xlabel('Best Output Value')
        plt.ylabel('KDE Value at Best Location')
        plt.title('KDE Value vs Best Output')
        
        os.makedirs(traj_path, exist_ok=True)
        plt.savefig(traj_path / f'plasticity_kde_{experiment_name}.png')
    
    
@comparison_registry.register('mean')
def plot_mean(cb:ComparisonBase, save_path:Path):
    traj_path = save_path / 'best'
    plt.figure(figsize=(12, 6))
    best_outputs = []
    for experiment_name, datasets in cb._all_data.items():
        best_output = []
        model = None
        for dataset_name, dataset in datasets.items():
            model = cb._data_infos[experiment_name][dataset_name]['additional_config']['Model']
            objective_name = [i['name'] for i in cb._data_infos[experiment_name][dataset_name]['objectives']]
            data = cb._all_data[experiment_name][dataset_name]
            Y = np.array([i[objective_name[0]] for i in data])
            best_output.append(np.min(Y))
        iters = np.array(list(range(len(best_output))))
        name = f"{model}"
    ...
    mean = np.mean(best_outputs, axis=0)
    std = np.std(best_outputs, axis=0)
    
    plt.plot(iters, mean, label=name)
    plt.fill_between(iters, mean-std, mean+std, alpha=0.2)
    plt.legend()
    os.makedirs(traj_path, exist_ok=True)
    plt.savefig(traj_path / 'mean.png')
    
@comparison_registry.register('best')
def compare_best(cb:ComparisonBase, save_path:Path):
    traj_path = save_path / 'best'
    plt.figure(figsize=(12, 6))
    for experiment_name, datasets in cb._all_data.items():
        best_output = []
        ...
        iters = np.array(list(range(len(best_output))))
        name = f"{model}"
        plt.plot(iters, best_output, label=name)
    plt.legend()
    os.makedirs(traj_path, exist_ok=True)
    plt.savefig(traj_path / 'best.png')
