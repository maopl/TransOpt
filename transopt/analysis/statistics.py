...
@comparison_registry.register('wilconxon')
def compare_best(cb:ComparisonBase, save_path:Path):
    wilconxon_path = save_path / 'wilconxon'
    os.makedirs(wilconxon_path, exist_ok=True)
    
    # Collect best outputs for each experiment
    experiment_results = {}
    for experiment_name, datasets in cb._all_data.items():
        ...
        for dataset_name, dataset in datasets.items():
            ...
            best_outputs.append(np.min(Y))
        experiment_results[experiment_name] = best_outputs

    # Perform Wilcoxon rank sum test between each pair of experiments
    experiment_names = list(experiment_results.keys())
    n_experiments = len(experiment_names)
    
    # Create results table
    results_table = np.zeros((n_experiments, n_experiments))
    p_values = np.zeros((n_experiments, n_experiments))
    
    for i in range(n_experiments):
        for j in range(i+1, n_experiments):
            stat, p_value = scipy.stats.ranksums(
                experiment_results[experiment_names[i]], 
                experiment_results[experiment_names[j]]
            )
            results_table[i,j] = stat
            results_table[j,i] = -stat
            p_values[i,j] = p_value
            p_values[j,i] = p_value
            
    # Save results to file
    with open(wilconxon_path / 'wilconxon_results.txt', 'w') as f:
        f.write("Wilcoxon Rank Sum Test Results:\n\n")
        for i in range(n_experiments):
            for j in range(i+1, n_experiments):
                f.write(f"{experiment_names[i]} vs {experiment_names[j]}:\n")
                f.write(f"Statistic: {results_table[i,j]:.4f}\n")
                f.write(f"p-value: {p_values[i,j]:.4f}\n")
                if p_values[i,j] < 0.05:
                    f.write("Significant difference detected (p < 0.05)\n")
                f.write("\n")
    plt.savefig(wilconxon_path / 'wilconxon.png')
                 


@comparison_registry.register('sk')
def plot_sk(ab:AnalysisBase, save_path:Path):
    sk_path = save_path / 'sk'
    ...
    for experiment_name, datasets in ab._all_data.items():
        best_outputs = []
        for dataset_name, dataset in datasets.items():
            objective_name = [i['name'] for i in ab._data_infos[experiment_name][dataset_name]['objectives']]
            data = ab._all_data[experiment_name][dataset_name]
            Y = np.array([i[objective_name[0]] for i in data])
            best_outputs.append(np.min(Y))
        experiment_results[experiment_name] = best_outputs

    # Perform Scott-Knott test
    df = pds.DataFrame(experiment_results)
    ...
    plt.title('Scott-Knott Test Results')
    plt.xlabel('Algorithm')
    plt.ylabel('Rank')
    plt.xticks(rotation=45)
    
    # Save plot
    plt.savefig(sk_path / 'sk.png', bbox_inches='tight')
    plt.close()
    
    # Save text results
    with open(sk_path / 'sk_results.txt', 'w') as f:
        f.write("Scott-Knott Test Results:\n\n")
