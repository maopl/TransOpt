...
const ProblemList = ({ 
  ...
  setCurrentNodePath
    ...
    const onSelect = (selectedKeys) => {
      setCurrentNodePath(selectedKeys);
    };
     ...
     */
    useEffect(() => {
        const [experimentIndex, taskIndex] = currentNodePath[0].split('-').map(Number);

        // 如果存在taskIndex那么点击的是问题, 否则点击的是实验, 就选择实验下的第一个问题
        const currentProblem = filteredExperiments[experimentIndex]?.problemList[taskIndex ?? 0];
  ...
  const titleRender = (nodeData) => {
    ...
    const isExperiment = nodeData.key.indexOf('-') === -1;
      const [currentExperimentIndex, currentTaskIndex] = currentNodePath[0].split('-').map(Number);
    // 处理实验节点
    if (isExperiment) {
